export const GOOGLE_API_KEY='AIzaSyD9EfpZU1-IlW1RQbgYzYyVqCqMhV66_HU';
export const SERVER_URL_PLACEHOLDER = '@LZ_SERVER_URL@';
export const ROOT_URL = process.env.REACT_APP_BACKEND_ENDPOINT || 'http://localhost:8080';
// export const ROOT_URL = 'https://peaceful-lowlands-11080.herokuapp.com';
export const GOOGLE_URL = 'https://maps.googleapis.com/maps/api/place/details/json';
export const INPUT_SEARCH_DELAY =  270;

export const ROLE_ADMIN =  'ROLE_ADMIN';
export const ROLE_USER =  'ROLE_USER';
export const ROLE_TRUSTED_USER =  'ROLE_TRUSTED_USER';

export const POST_TYPE_ARTICLE = 'POST_TYPE_ARTICLE';
export const POST_TYPE_EVENT = 'POST_TYPE_EVENT';

export const BORDER_EVENT = 'B_EVENT';
export const BORDER_ARTICLE = 'B_ARTICLE';
export const BORDER_SIDEBAR = 'B_SIDEBAR';

export const LM_GPS_COORDS = {lat: 49.0811, lng: 19.6192};

export const UPCOMING_EVENTS_COUNT = 10;
export const POSTS_PER_PAGE = 4;
export const PAGINATION_NUMBER_SPREAD = 2;

export const STRING_PROXIMITY_THRESHOLD = 3;